...
class UfoCacheFsCest
{
    ...
    public function saveTest(\CodeGuy $I) {
        ...
        $cache = new UfoCacheFs($this->testDataHash, $this->cacheSettings, $this->errors);
    ...
    public function saveLargeData(\CodeGuy $I) {
        ...
        $cache = new UfoCacheFs($this->testLargeDataHash, $this->cacheSettings, $this->errors);
    ...
    public function saveComplexHash(\CodeGuy $I) {
        ...
        $cache = new UfoCacheFs($this->testComplexHash, $this->cacheSettings, $this->errors);
        $I->executeMethod($cache, 'save', $this->testData);
    }
    ...
    public function loadNonExisting(\CodeGuy $I) {
        ...
        $cache = new UfoCacheFs($this->testNonExistingHash, $this->cacheSettings, $this->errors);
        $I->executeMethod($cache, 'load');
        $I->seeMethodReturns($cache, 'load', '');
    }
    
    public function deleteOld(\CodeGuy $I) {
        $this->showTest(__FUNCTION__);
        $cache = new UfoCacheFs($this->testDataHash, $this->cacheSettings, $this->errors);
        $I->executeMethod($cache, 'deleteOld');
    }
}
